...
; -> (listof (list delta-type symbol symbol natural any))
(define (entire-history struct)
  (g:collect (g:map (lambda (delta)
                      (list (audit-delta-type delta)
                            (entity-name    (audit-delta-entity delta))
                            (attribute-name (audit-delta-attribute delta))
                            (audit-delta-struct-id delta)
                            (audit-delta-value delta)))
                    (g:select #:from  audit-delta
                              #:where (and (= audit-delta.struct-id ,(snooze-struct-id struct))
                                           (in audit-delta.attribute-id ,(map attribute->id (entity-attributes (snooze-struct-entity struct)))))
                              #:order ((asc audit-delta.guid))))))

(define-check (check-entire-history expected)
  (check-equal? (struct-history struct) expected))

; snooze-struct -> (listof (list delta-type symbol symbol natural any))
(define (struct-history struct)
  (g:collect (g:map (lambda (delta)
                      (list (audit-delta-type delta)
                            (attribute-name (audit-delta-attribute delta))
                            (audit-delta-value delta)))
...
(define-check (check-struct-history struct expected)
  (check-equal? (struct-history struct) expected))
...
(define/provide-test-suite all-audit-tests
  ...
  (test-case "audit-attributes generated correctly"
    (for ([entity (in-list entities)])
      (for ([attr (in-list (entity-attributes entity))])
        (with-check-info (['table  (entity-table-name entity)]
  ...
  (test-case "audit basic insert, update and delete"
    ...
    (let ([person1 (save! (make-person "Dave"))]
          [id1     (snooze-struct-id person1)])
      
      (check-equal? (length (find-txns))  1)
      (check-equal? (length (find-metas)) 1)
