...
(define (make-audit-tests snooze)
  ...
  (test-suite "audit"
    ...
    (test-case "init-audit-trail!"
      ...
      (check-pred procedure? (ormap audit-stage? (send snooze get-transaction-pipeline)) "check 6")
      (check-pred procedure? (ormap audit-stage? (entity-insert-pipeline entity:person)) "check 7")
      (check-pred procedure? (ormap audit-stage? (entity-update-pipeline entity:person)) "check 8")
      (check-pred procedure? (ormap audit-stage? (entity-delete-pipeline entity:person)) "check 9")
      (check-pred procedure? (ormap audit-stage? (entity-insert-pipeline entity:pet)) "check 10")
      (check-pred procedure? (ormap audit-stage? (entity-update-pipeline entity:pet)) "check 11")
      (check-pred procedure? (ormap audit-stage? (entity-delete-pipeline entity:pet)) "check 12"))
