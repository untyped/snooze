(module query-lang mzscheme
  ...
  (define q:select
    (lambda/kw (#:key [alias (gensym 'select)] [what no-what-clause] from [where #f] [group null] [order null] [limit #f] [offset #f])
      ...
      (let-values ([(what-entities)           (what->entities what)]
    ...
    (case-lambda
      ...
      [(alias entity name)
       (let ([era-attribute (era:get-attribute (entity-entity entity) name)])
         ...
         (make-field (if alias 
                         alias
                         (symbol-append (named-alias entity) '- name))
                     entity
                     name
  ...
  (define (entity->fields entity)
    (cons* (q:field entity 'id type:id)
           (q:field entity 'revision type:revision)
           (era:map-attributes/entity (cut q:field entity <> <>)
