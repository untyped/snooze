...
(require scheme/class
         ...
         (file "../snooze-mzscheme.ss")
         (file "attribute.ss")
         (file "cache.ss"))

(define/provide-persistent-struct audit-delta
    ...
    (define/public (revert-delta! guid delta struct)
      ...
      (if (eq? (audit-delta-type delta) 'I)
          ...
          (let ([struct (if struct struct (make-persistent-struct/defaults (guid-entity guid)))]
            ...
            (set-struct-attribute! struct (attribute-column-name attr) (audit-delta-value delta (attribute-type attr)))
            struct)))
