(module sql-quote-unit-test mzscheme
  ...
  (define sql-quote-unit-tests
    (test-suite
     ...
     (test-equal?
      "quote-id wraps symbol id in `back ticks`"
      (quote-id 'my-id)
      "`my-id`")
     
     (test-equal?
      "quote-id wraps string id in `back ticks`"
      (quote-id "my-id")
      "`my-id`")
     ...
     (test-case
      ...
      (check equal? (unquote-data type:boolean "1") #t)
      (check equal? (unquote-data type:boolean "0") #f))
