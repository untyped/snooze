...
@definterface[snooze<%> ()]{
  ...
  @schemeid[query] is a @schemeid[select] statement, as generated by an @scheme[(sql (select ...))] block. It determines both the data retrieved and the type of each @schemeid[result].}
  ...
  @defmethod[(g:find [query query?]) (gen-> result)]{@margin-note{@other-manual['(planet untyped/unlib:3/gen)]} Similar to @scheme[find-all], but returns a @italic{generator} of results. This is the most general query mechanism offered by Snooze: generators allow you to manipulate results one at a time in a functional manner, without wasting lots of memory on intermediate lists.}
