  ...
  (let ([guid      (car row)]
    ...
    (values 
     ...
     (and guid
          ...
          (or (and (send (send snooze get-current-cache) cache-ref guid) guid)
              ; Else parse the data in the row:
              (send snooze cache-add! (apply (entity-private-constructor entity) guid (cdr (take row num-attrs))))))
     (drop row num-attrs))))
