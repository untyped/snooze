     ...
     (test-case
      ...
      (let* ([table1 (q:table 't1 'table1)]
        ...
        (check-equal? (get-sql display-expr-sql (q:in field1 (q:select #:what (list field2) #:from table2)) (list field1))
                      ...
                      "IN statement with select")
        (check-equal? (get-sql display-expr-sql (q:in field1 '(1 2 3)) (list field1))
                      "(`f1` IN (1, 2, 3))"
                      "IN statement with non-null value list")
        (check-equal? (get-sql display-expr-sql (q:in field1 null) (list field1))
                      "(false)"
                      "IN statement with null value list")))
